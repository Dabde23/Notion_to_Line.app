...
class NotionClient:
    ...
    def get_month_schedule(self, year: int, month: int) -> list[dict]:
        ...
        query = {
            ...
            "sorts": [{"property": NotionProperties.DATE, "direction": "ascending"}]
        }
        has_more = True
        next_cursor = None
        page_count = 0
...
def extract_text(results: list[dict]) ->list[ScheduleItem]:
    ...
    for page in results:
        ...
        title_info = props.get(NotionProperties.SITE_NAME, {}).get("title", None)
        title = title_info[0].get("plain_text") if title_info else None

        tag_info = props.get(NotionProperties.COMPANY, {}).get("multi_select", None)
        tag = tag_info[0].get("name") if tag_info else None
...
def group_by_tag(text_list: list[ScheduleItem]) -> dict[str, list[ScheduleItem]]:
    #グルーピング
...
def format_plain_text(grouped_data: dict[str, list[ScheduleItem]]) -> str:
...
class LineClient:
    ...
    def _handle_response(response):
        ...
        if response.status_code >= 500:
            raise LineServerError(response.status_code)
...
class LineTransientError(LineError):
    def __init__(self, status_code: int | None = None, message: str | None = "接続エラー"):
        self.status_code = status_code
...
class LineServerError(LineTransientError):
    def __init__(self, status_code: int) -> None:
        self.status_code = status_code
        super().__init__(status_code, f"Lineサーバーエラー: {status_code}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        grouped_data = group_by_tag(text_list)
        final_text = format_plain_text(grouped_data)
    except  NotionError as e:
        print(e)
        sys.exit(1)
